...
class Diffusion:
    def __init__(self, noise_steps=1000, beta_start=1e-4, beta_end=0.02, batch_size=1, channels=1, img_size=256, device="cuda", run_name="ddpm", model_ckpt=None, ema_model_ckpt=None):
        ...
        self.noise_scheduler = DDPMScheduler(num_train_timesteps=self.noise_steps)
    ...
    def noise_images(self, x, t):
        ...
        # Create noise of the same shape as x
        ...
        # Return the noised image and the noise
        return x_t, Ɛ
    ...
    def sample(self, use_ema, n, condition):
        model = self.ema_model if use_ema else self.model
        logging.info(f"Sampling {n} new images....")
        with torch.no_grad():
            x = torch.randn((n, 1, self.img_size, self.img_size)).to(self.device)
            if not isinstance(condition, torch.Tensor):
                condition = torch.zeros_like(x).to(self.device)
            
            self.noise_scheduler.set_timesteps(self.noise_steps)

            for t in progress_bar(self.noise_scheduler.timesteps):
                model_output = model(torch.cat((x, condition), dim=1), t).sample
    ...
    def one_epoch(self, train=True):
        ...
        for step, data in enumerate(pbar):
            with torch.autocast("cuda") and (torch.inference_mode() if not train else torch.enable_grad()):
                clean_images = data['images'].to(self.device)

                images_degraded = data['images_degraded'].to(self.device)
                p = random.uniform(0, 1)
                if p <= 0.5:
                    images_degraded = torch.zeros_like(clean_images).to(self.device)
                ...
                x_t = self.noise_scheduler.add_noise(clean_images, noise, timesteps)

                predicted_noise = self.model(torch.cat((x_t, images_degraded), dim=1), timesteps).sample
    ...
    def log_images(self, epoch):
        ...
        images_degraded = next(iter(self.val_dataloader))['images_degraded'].to(self.device)
        ema_sampled_images = self.sample(use_ema=True, n=self.batch_size, condition=images_degraded)
        image_grid = make_grid(ema_sampled_images, rows=self.batch_size, cols=1)
        image_grid.save(os.path.join("results", self.run_name, f"guidance_{epoch:04d}.png"))
        # EMA model sampling without guidance
        ema_sampled_images = self.sample(use_ema=True, n=self.batch_size, condition=None)
        image_grid = make_grid(ema_sampled_images, rows=self.batch_size, cols=1)
        image_grid.save(os.path.join("results", self.run_name, f"no_guidance_{epoch:04d}.png"))
