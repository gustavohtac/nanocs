    ...
    def sample(self, use_ema, n, condition):
        ...
        with torch.no_grad():
            ...
            for i in progress_bar(reversed(range(1, self.noise_steps)), total=self.noise_steps-1, leave=False):
                ...
                predicted_noise = model(torch.cat((x, condition), dim=1), t).sample
    ...
    def one_epoch(self, train=True):
        ...
        for step, data in enumerate(pbar):
            with torch.autocast("cuda") and (torch.inference_mode() if not train else torch.enable_grad()):
                ...
                if p <= 0.5:
                    images_degraded = torch.zeros_like(images).to(self.device)

                t = self.sample_timesteps(images.shape[0]).to(self.device)
                x_t, noise = self.noise_images(images, t)
                predicted_noise = self.model(torch.cat((x_t, images_degraded), dim=1), t).sample
